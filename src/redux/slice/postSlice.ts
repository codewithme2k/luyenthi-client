...
import { callFetchPost, callCreatePost, callUpdatePost, callDeletePost } from '@/config/api'
import type { IPost } from '@/types/backend'

/* =====================
   Thunk
===================== */
export const fetchPost = createAsyncThunk('post/fetchPost', async ({ query }: { query?: string }) => {
  const res = await callFetchPost(query)
  return res.data
})

export const createPost = createAsyncThunk('post/createPost', async (post: Partial<IPost>) => {
...
export const deletePost = createAsyncThunk('post/deletePost', async ({ id }: { id: string }) => {
