...
export const UserActions: React.FC = () => {
  ...
  if (isAuthenticated) {
    return (
      <div className="relative z-50">
        ...
        >
          <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center overflow-hidden transition-all duration-300 shadow-sm ${
            user?.isPremium 
          ...
          <span className={`max-w-[100px] truncate hidden md:inline-block ${user?.isPremium ? "text-amber-500 font-bold" : "text-foreground"}`}>
            {user?.name || user?.email || "Người dùng"}
          </span>
          {user?.isPremium && (
            <span className="text-[10px] font-black text-amber-500 bg-amber-500/15 px-1.5 py-0.5 rounded border border-amber-500/20 scale-90 hidden md:inline-block leading-none">VIP</span>
          )}
          <ChevronDown className={`w-4 h-4 text-muted-foreground hidden md:inline-block transition-transform duration-300 ${dropdownOpen ? "rotate-180" : ""}`} />
        </button>
          ...
          <>
            ...
            <div className="absolute right-0 mt-2 w-52 rounded-2xl border border-border/60 bg-popover/90 backdrop-blur-md p-2 shadow-2xl z-50 animate-fade-in font-medium text-sm">
              <div className="px-3 py-2 border-b border-border mb-1 text-xs text-muted-foreground flex justify-between items-center">
                ...
                {user?.isPremium && (
                  <span className="text-[10px] font-extrabold text-amber-500 flex items-center gap-0.5"><Crown className="w-3 h-3 fill-current" /> VIP</span>
                )}
              </div>
  ...
  return (
    <div className="flex items-center gap-2">
      <Link to="/login">
        <Button className="btn-premium h-10 px-5 rounded-xl font-extrabold cursor-pointer">
          Đăng Nhập
        </Button>
      </Link>
    </div>
