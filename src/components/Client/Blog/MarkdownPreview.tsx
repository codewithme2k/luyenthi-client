...
export const MarkdownPreview: React.FC<MarkdownPreviewProps> = ({ content }) => {
  const html = useMemo(() => {
    ...
    parsed = parsed.replace(
      /`([^`]+)`/g,
      '<code class="px-2 py-0.5 rounded bg-muted font-mono text-xs text-primary font-bold">$1</code>'
    )
    ...
    parsed = parsed.replace(
      /&gt;\s*(.*)/g,
      '<blockquote class="border-l-4 border-primary/40 pl-5 py-1.5 italic my-6 text-muted-foreground">$1</blockquote>'
    )

    // Headings
    parsed = parsed.replace(
      /^# (.*$)/gim,
      '<h1 class="text-3xl font-extrabold font-heading text-foreground mt-8 mb-4 border-b border-border pb-2.5">$1</h1>'
    )
    parsed = parsed.replace(
      /^## (.*$)/gim,
      '<h2 class="text-2xl font-bold font-heading text-foreground mt-7 mb-3 pb-1">$1</h2>'
    )
    parsed = parsed.replace(
      /^### (.*$)/gim,
      '<h3 class="text-xl font-semibold font-heading text-foreground mt-6 mb-2">$1</h3>'
    )
    ...
    parsed = parsed.replace(
      /^\s*-\s+(.*$)/gim,
      '<li class="ml-4 list-disc pl-2 text-sm my-1.5 leading-relaxed text-foreground">$1</li>'
    )
    parsed = parsed.replace(
      /^\s*\*\s+(.*$)/gim,
      '<li class="ml-4 list-disc pl-2 text-sm my-1.5 leading-relaxed text-foreground">$1</li>'
    ...
    parsed = parsed.replace(
      /^\s*\d+\.\s+(.*$)/gim,
      '<li class="ml-4 list-decimal pl-2 text-sm my-1.5 leading-relaxed text-foreground">$1</li>'
    )
