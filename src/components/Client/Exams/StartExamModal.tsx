...
export const StartExamModal: React.FC<StartExamModalProps> = ({ exam, onClose, onConfirm }) => {
  return (
      ...
      <div className='glass bg-background sm:max-w-[480px] w-full rounded-2xl overflow-hidden shadow-2xl animate-fade-in border border-border'>
        <div className='p-6 space-y-6'>
          ...
          <div className='text-xs text-muted-foreground leading-relaxed text-center font-medium'>
            ⚠️ **Lưu ý**: Sau khi bấm bắt đầu, đồng hồ đếm ngược sẽ bắt đầu chạy và bạn không thể tạm dừng. Hãy chắc
            chắn đường truyền Internet hoạt động ổn định.
          </div>

          {/* Action Buttons */}
          <div className='flex gap-3 border-t border-border pt-4'>
            <Button onClick={onClose} variant='outline' className='flex-1 h-11 rounded-xl font-bold cursor-pointer'>
              Hủy bỏ
            </Button>
            <Button onClick={onConfirm} className='btn-premium flex-1 h-11 rounded-xl font-bold cursor-pointer'>
              Bắt Đầu Ngay
            </Button>
          </div>
        </div>
