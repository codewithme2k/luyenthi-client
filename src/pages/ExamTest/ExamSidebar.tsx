...
export const ExamSidebar: React.FC<ExamSidebarProps> = ({ questions, answers, flags, currentIndex, onNavigate }) => {
  ...
  return (
    <div className='glass p-5 rounded-2xl border border-border flex flex-col h-[calc(100vh-8rem)] min-h-[450px] shadow-sm'>
      ...
      <div className='flex-1 min-h-0 overflow-y-auto scrollbar-thin pr-3 -mr-3'>
        <div className='grid grid-cols-5 md:grid-cols-6 gap-3 pb-2 mt-2 mx-2'>
          {questions.map((q, index) => {
            ...
            return (
              <button
                ...
                className={`
            w-8 h-8 flex flex-col items-center justify-center rounded-full text-sm font-bold transition-all duration-200 cursor-pointer border relative
            ${isCurrent ? 'ring-2 ring-primary ring-offset-2 ring-offset-background scale-110 z-10 shadow-md' : 'hover:scale-105 hover:bg-muted'}
            ${
              hasAnswered
          ...
          `}
              >
                {index + 1}
                {isFlagged && (
                  <div className='absolute top-0 right-0 -translate-y-1/4 translate-x-1/4 w-3.5 h-3.5 text-[8px] flex items-center justify-center bg-amber-100 rounded-full shadow-sm z-20'>
                    🚩
                  </div>
                )}
              </button>
