  ...
  return (
    <div className='container mx-auto px-4 py-8 max-w-4xl animate-fade-in'>
      ...
      <div className='glass p-6 sm:p-8 rounded-3xl border border-border shadow-[0_10px_30px_rgba(0,0,0,0.02)] mb-8 relative overflow-hidden'>
        ...
        <div
          className={`absolute -right-24 -top-24 w-64 h-64 rounded-full blur-[100px] pointer-events-none opacity-20 ${isPassed ? 'bg-green-500' : 'bg-destructive'}`}
        />

        <div className='flex flex-col md:flex-row items-center justify-between gap-6 relative z-10'>
          <div className='text-center md:text-left space-y-3'>
            <span
              className={`inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${isPassed ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-destructive/10 text-destructive border border-destructive/20'}`}
            >
              {isPassed ? (
                <>
            ...
            <h1 className='text-2xl sm:text-3xl font-extrabold font-heading text-foreground'>{exam.title}</h1>
          ...
          <div className='flex-shrink-0 flex flex-col items-center justify-center p-6 rounded-2xl bg-muted/40 border border-border/80 min-w-[150px] shadow-sm text-center'>
            ...
            <div className='flex items-baseline gap-0.5'>
              ...
              <span className='text-base text-muted-foreground font-semibold'>/{exam.totalMarks}</span>
            </div>
            <span className='text-[11px] text-muted-foreground/80 mt-1.5 font-medium'>
              Điểm chuẩn: {exam.passMarks} điểm
            </span>
          </div>
        </div>
      </div>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-8'>
        ...
        <div className='p-4 rounded-2xl bg-green-500/5 border border-green-500/20 flex flex-col items-center justify-center gap-1.5 text-center'>
          <span className='text-xs text-green-600 font-bold uppercase tracking-wider flex items-center gap-1 justify-center'>
        ...
        <div className='p-4 rounded-2xl bg-destructive/5 border border-destructive/20 flex flex-col items-center justify-center gap-1.5 text-center'>
          <span className='text-xs text-destructive font-bold uppercase tracking-wider flex items-center gap-1 justify-center'>
        ...
        <div className='p-4 rounded-2xl bg-blue-500/5 border border-blue-500/20 flex flex-col items-center justify-center gap-1.5 text-center'>
          <span className='text-xs text-blue-600 font-bold uppercase tracking-wider flex items-center gap-1 justify-center'>
            <HelpCircle className='w-3.5 h-3.5' /> Chờ chấm
          </span>
      ...
      {resultStats.pending > 0 && (
        <div className='mb-8 p-4 rounded-2xl bg-amber-500/10 text-amber-600 border border-amber-500/20 text-sm font-medium flex items-start gap-2.5'>
          ...
          <div>
            Có <strong>{resultStats.pending} câu hỏi tự luận/điền khuyết</strong> chưa được chấm điểm hoặc đang đợi giáo
            viên chấm điểm thủ công. Điểm số chính thức của bạn có thể thay đổi sau khi có kết quả hoàn chỉnh.
          </div>
        </div>
      )}
        ...
        {filteredUserAnswers.length > 0 ? (
          filteredUserAnswers.map((ans: any, idx: number) => {
            ...
            return (
              <div
                key={ans.id}
                className='glass p-6 sm:p-8 rounded-2xl border border-border shadow-sm flex flex-col gap-4 relative transition-all hover:shadow-md'
              >
                ...
                {/* Question Title */}
                <div
                  className='text-base sm:text-lg font-semibold leading-relaxed text-foreground'
                  ...
                  <div className='space-y-2.5 mt-2'>
                    {options.map((opt: any) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-5 h-5 rounded-full border flex items-center justify-center flex-shrink-0 ${indicatorColor}`}
                          >
                            {iconNode}
                          </div>
                  ...
                  <div className='space-y-2.5 mt-2'>
                    {options.map((opt: any) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-5 h-5 border flex items-center justify-center flex-shrink-0 ${indicatorColor}`}
                          >
                            {iconNode}
                          </div>
                          <span className='flex-1 leading-relaxed'>{opt.content}</span>
