  ...
  const result = useMemo(() => {
    ...
    questions.forEach((q) => {
      ...
      if (q.type === 'SINGLE_CHOICE' || q.type === 'TRUE_FALSE') {
        ...
        const isExactMatch =
          correctOptions.length === userAnswers.length && correctOptions.every((id) => userAnswers.includes(id))
    ...
    <div className='container mx-auto px-6 py-12 max-w-3xl animate-fade-in'>
      <div className='glass p-8 sm:p-12 rounded-3xl border border-border text-center'>
        ...
        <div className='mb-8 relative inline-block'>
          <div
            className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto ${result.isPassed ? 'bg-green-500/10 text-green-500' : 'bg-destructive/10 text-destructive'}`}
          >
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-10'>
          ...
          <div className='p-4 rounded-2xl bg-green-500/5 border border-green-500/20 flex flex-col items-center justify-center gap-2'>
            <span className='text-sm text-green-600 font-semibold uppercase tracking-wider flex items-center gap-1'>
          ...
          <div className='p-4 rounded-2xl bg-destructive/5 border border-destructive/20 flex flex-col items-center justify-center gap-2'>
            <span className='text-sm text-destructive font-semibold uppercase tracking-wider flex items-center gap-1'>
          ...
          <div className='p-4 rounded-2xl bg-blue-500/5 border border-blue-500/20 flex flex-col items-center justify-center gap-2'>
            <span className='text-sm text-blue-600 font-semibold uppercase tracking-wider flex items-center gap-1'>
        ...
        {result.pending > 0 && (
          <div className='mb-10 p-4 rounded-xl bg-amber-500/10 text-amber-600 border border-amber-500/20 text-sm font-medium'>
            ...
            Có {result.pending} câu hỏi tự luận/điền khuyết đang chờ giáo viên chấm điểm thủ công. Điểm số trên có thể
            thay đổi.
          </div>
        )}
        ...
        <div className='flex flex-col sm:flex-row gap-4 justify-center mt-8'>
          <Button
            ...
            variant='outline'
            className='border-border hover:bg-muted h-12 px-8 rounded-xl font-bold gap-2'
          >
          ...
          {sessionId && (
            <Button
              onClick={() => navigate(`/exam-result/${sessionId}`)}
              className='btn-premium h-12 px-8 rounded-xl font-bold gap-2'
            >
