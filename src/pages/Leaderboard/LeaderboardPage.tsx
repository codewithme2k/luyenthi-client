...
export default function LeaderboardPage() {
  ...
  const aggregatedStudents = useMemo<LeaderboardStudent[]>(() => {
    const studentMap: Record<
      string,
      { name: string; email: string; sessions: any[]; avatarImg?: string; isPremium?: boolean }
    > = {}
  ...
  const rankedStudents = useMemo(() => {
    let filtered = aggregatedStudents.filter(
      (student) =>
  ...
  return (
    <div className='container max-w-7xl mx-auto py-10 px-4 sm:px-6 space-y-8 page-bg'>
      ...
      {rankedStudents.length > 0 && !searchTerm && (
        <LeaderboardPodium top1={top1} top2={top2} top3={top3} getImageUrl={getImageUrl} />
      )}

      {/* 4. Complete Runners-up List Table */}
      <LeaderboardTable rankedStudents={rankedStudents} searchTerm={searchTerm} getImageUrl={getImageUrl} />
    </div>
  )
}
