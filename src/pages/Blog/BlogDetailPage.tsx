...
import { ArrowLeft, Calendar, Clock, User, Share2, Heart, Bookmark, Sparkles, BookOpen } from 'lucide-react'
  ...
  if (!post) {
    return (
      <div className='min-h-screen bg-background py-20 text-center space-y-6'>
        ...
        <Link to='/blog'>
          <Button className='btn-premium h-11 px-6 rounded-xl font-bold cursor-pointer mt-4'>Quay Lại Cẩm Nang</Button>
        </Link>
      </div>
    )
  }
      ...
      <div className='container mx-auto px-6 max-w-7xl pt-10 relative'>
        ...
        <div className='mb-8 animate-fade-in shrink-0'>
          <Link to='/blog'>
            <Button
              variant='ghost'
              className='hover:bg-muted font-bold text-xs gap-1.5 px-3 py-1.5 rounded-xl cursor-pointer text-muted-foreground hover:text-foreground transition-colors border border-border/40 bg-background/30 backdrop-blur-xs'
            >
        ...
        <article className='glass rounded-3xl border border-border/80 p-6 sm:p-10 space-y-8 animate-fade-in shadow-xl bg-background/60 backdrop-blur-md'>
          ...
          <div className='space-y-4'>
            ...
            <h1 className='text-2xl sm:text-4xl font-black font-heading leading-tight text-foreground'>{post.title}</h1>
            ...
            <div className='flex items-center gap-2.5'>
              <Button
                ...
                className={`flex items-center gap-1.5 px-4 h-10 rounded-xl font-bold transition-all cursor-pointer ${
                  isLiked ? 'bg-rose-500/10 border-rose-500/30 text-rose-600 hover:text-rose-700' : 'hover:bg-muted'
                }`}
              >
              ...
              <Button
                ...
                onClick={handleSave}
                className={`flex items-center gap-1.5 px-4 h-10 rounded-xl font-bold transition-all cursor-pointer ${
                  isSaved ? 'bg-amber-500/10 border-amber-500/30 text-amber-600 hover:text-amber-700' : 'hover:bg-muted'
                }`}
              >
