...
import { callFetchCourses, callCreateCourse, callUpdateCourse, callDeleteCourse, callFetchCategory } from '@/config/api'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Plus, Search, Edit, Trash2, Layers, Crown, Eye, EyeOff, GraduationCap, Sparkles, Check, X } from 'lucide-react'
...
export const AdminCoursePage: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm(
        'Bạn có chắc chắn muốn xoá khoá học này? Mọi chương mục và bài học liên quan sẽ bị xoá vĩnh viễn!'
      )
    ) {
      return
    }
    ...
    <div className='p-6 space-y-6 w-full max-w-7xl mx-auto page-bg animate-fade-in'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
        ...
        <Button
          ...
          className='btn-premium rounded-xl cursor-pointer font-bold flex items-center gap-1.5 shadow-md'
        >
            ...
            <tbody className='divide-y divide-border/60 text-xs font-semibold'>
              {courses.map((course) => (
                <tr key={course.id} className='hover:bg-muted/10 transition-colors'>
                  ...
                  <td className='p-4'>
                    <div className='flex items-center gap-3'>
                      ...
                      <div className='min-w-0'>
                        <p className='font-extrabold text-foreground truncate max-w-xs sm:max-w-md'>{course.title}</p>
                  ...
                  {/* Chapter count */}
                  <td className='p-4 text-center font-bold text-foreground'>{course._count?.chapters || 0} chương</td>
