...
const MarkdownPreview = ({ content }: { content: string }) => {
  const html = useMemo(() => {
    if (!content)
      return "<p class='text-muted-foreground italic text-center py-10'>Chưa có nội dung xem trước. Hãy nhập nội dung Markdown bên trái...</p>"
    ...
    parsed = parsed.replace(
      /`([^`]+)`/g,
      '<code class="px-1.5 py-0.5 rounded bg-muted font-mono text-xs text-primary font-bold">$1</code>'
    )
    ...
    parsed = parsed.replace(
      /&gt;\s*(.*)/g,
      '<blockquote class="border-l-4 border-primary/40 pl-4 py-1 italic my-4 text-muted-foreground">$1</blockquote>'
    )

    // Headings
    parsed = parsed.replace(
      /^# (.*$)/gim,
      '<h1 class="text-2xl font-extrabold font-heading text-foreground mt-6 mb-3 border-b border-border pb-2">$1</h1>'
    )
    parsed = parsed.replace(
      /^## (.*$)/gim,
      '<h2 class="text-xl font-bold font-heading text-foreground mt-5 mb-2.5 pb-1">$1</h2>'
    )
    parsed = parsed.replace(
      /^### (.*$)/gim,
      '<h3 class="text-lg font-semibold font-heading text-foreground mt-4 mb-2">$1</h3>'
    )
    ...
    parsed = parsed.replace(
      /^\s*-\s+(.*$)/gim,
      '<li class="ml-4 list-disc pl-1.5 text-sm my-1 leading-relaxed text-foreground">$1</li>'
    )
    parsed = parsed.replace(
      /^\s*\*\s+(.*$)/gim,
      '<li class="ml-4 list-disc pl-1.5 text-sm my-1 leading-relaxed text-foreground">$1</li>'
    ...
    parsed = parsed.replace(
      /^\s*\d+\.\s+(.*$)/gim,
      '<li class="ml-4 list-decimal pl-1.5 text-sm my-1 leading-relaxed text-foreground">$1</li>'
    )
          ...
          <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-5 md:px-8 border-b border-border bg-muted/20 shrink-0'>
            ...
            <div className='flex items-center gap-3 w-full md:w-auto justify-end'>
              ...
              <Button
                type='submit'
                disabled={isFetching}
                className='font-extrabold cursor-pointer h-10 px-5 shadow-md shadow-primary/20'
              >
            ...
            <div className='flex-1 flex flex-col border-r border-border overflow-hidden bg-card'>
              ...
              <div className='flex items-center gap-1 p-2 border-b border-border bg-muted/30 shrink-0 overflow-x-auto'>
                ...
                <Button
                  ...
                  variant='ghost'
                  size='icon'
                  className='h-8-w-8 px-2 h-8 w-auto gap-1 text-[10px] font-black hover:bg-muted text-blue-600 dark:text-blue-400'
                  onClick={() => insertMarkdown('alert')}
                  title='Hộp thông tin'
                >
                ...
                <Button
                  ...
                  variant='ghost'
                  size='icon'
                  className='h-8-w-8 px-2 h-8 w-auto gap-1 text-[10px] font-black hover:bg-muted text-amber-600 dark:text-amber-400'
...
Bạn có thể viết công thức, chèn hình ảnh, bảng biểu và danh sách học tập cực kỳ dễ dàng.'
              ...
              <div className='px-4 py-2 border-t border-border bg-muted/20 shrink-0 flex items-center justify-between text-[10px] font-black text-muted-foreground uppercase tracking-wider'>
                <span className='flex items-center gap-1'>
                  <FileText className='w-3 h-3' /> Ký tự: {postContent.length}
                </span>
                <span className='flex items-center gap-1'>
                  <Clock className='w-3 h-3' /> Đọc ước tính:{' '}
                  {Math.max(1, Math.round(postContent.split(/\s+/).length / 200))} phút
                </span>
              </div>
            </div>
                ...
                <div className='max-w-2xl mx-auto space-y-4'>
                  ...
                  {watch('title') && (
                    <div className='space-y-3 pb-6 border-b border-border'>
                      ...
                      <h1 className='text-3xl font-black font-heading leading-tight text-foreground'>
                        {watch('title')}
                      </h1>
                      <div className='flex items-center gap-2 text-xs font-bold text-muted-foreground'>
                        <span className='px-2 py-0.5 rounded bg-muted'>
                          Tác giả: {dataUpdate?.author?.name || account?.user?.name || 'Admin'}
                        </span>
                        <span>•</span>
            ...
            {posts.length === 0 && !isFetching ? (
              ...
              posts.map((post) => (
                <TableRow key={post.id}>
                  ...
                  <TableCell>
                    <span
                      className={`px-2 py-1 rounded-full text-xs ${post.isPublished ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}
                    >
