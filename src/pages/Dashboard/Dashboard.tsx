...
export default function Dashboard() {
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      if (res.data?.success) {
        ...
        setMembershipRequests((prev) => prev.map((item) => (item.id === id ? { ...item, status: 'APPROVED' } : item)))
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      if (res.data?.success) {
        ...
        setMembershipRequests((prev) => prev.map((item) => (item.id === id ? { ...item, status: 'REJECTED' } : item)))
      } else {
  ...
  // VIP Conversion Rate
  const vipConversionRate = stats.totalUsers > 0 ? Math.round((stats.vipUsers / stats.totalUsers) * 100) : 0
        ...
        <div className='bg-card rounded-xl border border-border/80 p-6 flex flex-col justify-between shadow-xs'>
          ...
          <div className='flex justify-center items-center py-6'>
            <div className='relative w-36 h-36 flex items-center justify-center'>
              ...
              <svg className='w-full h-full transform -rotate-90'>
                <circle cx='72' cy='72' r='62' className='stroke-muted fill-none' strokeWidth='10' />
                <circle
                  cx='72'
                  cy='72'
                  r='62'
          ...
          <div className='text-center bg-amber-500/5 border border-amber-500/15 p-3 rounded-xl'>
            <p className='text-xs text-amber-600 font-bold leading-relaxed'>
              <strong>{stats.vipUsers}</strong> trong tổng số <strong>{stats.totalUsers}</strong> học sinh đã nâng cấp
              trọn gói học tập Premium VIP.
            </p>
          </div>
        </div>
      </div>
                ...
                .map((req) => (
                  ...
                  >
                    <div className='space-y-1 min-w-0'>
                      ...
                      <p className='text-xs font-black text-rose-500'>{formatCurrency(req.amount)}</p>
                    </div>
