    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setSelectedSession((prev: any) => {
          ...
          return {
            ...prev,
            userAnswers: prev.userAnswers.map((ans: any) => (ans.id === answerId ? { ...ans, isCorrect } : ans))
          }
        })
                ...
                <TableRow key={session.id}>
                  ...
                  <TableCell className='text-center'>
                    {session.score >= session.exam?.passMarks ? (
                      <span className='text-xs px-2.5 py-1 bg-green-500/10 text-green-600 rounded-md font-bold uppercase'>
                        Đạt
                      </span>
                    ) : (
                      <span className='text-xs px-2.5 py-1 bg-destructive/10 text-destructive rounded-md font-bold uppercase'>
                        Chưa Đạt
                      </span>
                    )}
                  </TableCell>
                  <TableCell className='text-center'>
                    <Button
                      ...
                      className='bg-blue-500/5 text-blue-600 border-blue-500/20 hover:bg-blue-500/10 hover:text-blue-700'
                    >
                  ...
                  return (
                    ...
                    >
                      <div className='flex gap-3 mb-3'>
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${isEssay ? 'bg-amber-100 text-amber-700 shadow-inner' : 'bg-primary/10 text-primary shadow-inner'}`}
                        >
                          {index + 1}
                        </div>
                        <div className='flex-1 pt-1'>
                          ...
                          <div className='mt-2 flex gap-2'>
                            <span className='px-2 py-0.5 bg-muted rounded-md text-xs font-semibold text-muted-foreground'>
                              {q.type}
                            </span>
                            <span className='px-2 py-0.5 bg-primary/10 text-primary rounded-md text-xs font-bold'>
                              {q.score || selectedSession.exam.totalMarks / selectedSession.userAnswers.length} điểm
                            </span>
                          </div>
                        </div>
                      </div>
                          ...
                          {q.type === 'SINGLE_CHOICE' || q.type === 'TRUE_FALSE' || q.type === 'MULTIPLE_CHOICE' ? (
                            <div className='space-y-2 mt-2'>
                              {q.options?.map((opt: any) => {
                                ...
                                return (
                                  <div
                                    key={opt.id}
                                    className={`p-3 rounded-lg border flex items-center gap-3 transition-all ${borderClass}`}
                                  >
                                    <div
                                      className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${isSelected ? 'bg-primary border-primary text-primary-foreground' : 'bg-background border-input'}`}
                                    >
                            ...
                            <div className='space-y-3'>
                              <div className='p-4 bg-muted/40 rounded-lg whitespace-pre-wrap font-medium text-foreground border border-border text-sm shadow-inner'>
                                {answer.textContent || (
                                  <span className='text-muted-foreground italic'>(Học sinh bỏ trống)</span>
                                )}
                              </div>
                              {q.type === 'FILL_BLANK' && (
                                <div className='p-3.5 bg-emerald-50/50 border border-emerald-200/60 rounded-lg text-xs font-bold text-emerald-800 flex items-center gap-2 shadow-xs'>
                                  ...
                                  <span>
                                    Đáp án mẫu đúng:{' '}
                                    <span className='underline decoration-2 bg-emerald-100/50 px-1.5 py-0.5 rounded'>
                                      {q.options?.find((opt: any) => opt.isCorrect)?.content || 'Chưa cấu hình'}
                                    </span>
                                  </span>
                                </div>
                              )}
